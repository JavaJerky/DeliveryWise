...
/**
 * <p>
 * Stub implementation of {@link RouteDataProvider} for Phase 1 MVP demonstrations.
 * </p>
 * <p>
 * Стаб-реалізація {@link RouteDataProvider} для демонстрації MVP на першому етапі.
 * </p>
 *
...
public class StubRouteDataProvider implements RouteDataProvider {
    ...
    public StubRouteDataProvider(ObjectMapper objectMapper,
    ...
    /**
     * Завантажує точки з файлу kyiv-test-orders.json, розраховує логістичні прапори та пакетно анонімізує весь список
     * перед передачею в оптимізатор.
     *
    ...
    public List<DeliveryPoint> getAnonymizedDeliveryPoints() {
        try {
            ...
            InputStream inputStream = new ClassPathResource(
                    "mock-data/kyiv-test-orders.json").getInputStream();
            ...
            throw new RuntimeException(
                    "Failed to load and process stub delivery points from mock-data/kyiv-test-orders.json",
                    e);
        }
    }

    /**
     * Завантажує список автомобілів з конфігураційного JSON-файлу стаб-даних та відфільтровує лише ті ТЗ, які сьогодні
     * готові до рейсу (статус READY).
     *
    ...
    public List<Vehicle> getAvailableFleet() {
        try {
            ...
            InputStream inputStream = new ClassPathResource(
                    "mock-data/fleet-config.json").getInputStream();
            ...
            throw new RuntimeException(
                    "Failed to load and filter fleet configuration from mock-data/fleet-config.json",
                    e);
        }
    }
