...
/**
 * <p>
 * Core engine for fleet routing optimization using Google OR-Tools.
 * </p>
 * <p>
 * Головний двигун оптимізації маршрутів автопарку на базі Google OR-Tools.
 * </p>
 *
...
public class RouteOptimizationService {
    ...
    /**
     * Constructs the optimization service with required data providers and default SaaS depot properties. Конструктор
     * сервісу оптимізації з необхідними провайдерами даних та параметрами депо за замовчуванням.
     */
    public RouteOptimizationService(
        ...
        this.defaultDepot = new Depot(0, depotName, depotAddress, depotLatitude,
                depotLongitude);
    }

    /**
     * Core orchestration method for routing calculation and VRP solving. Основний метод оркестрації розрахунку
     * маршрутів та розв'язання задачі VRP.
     */
    public void calculateOptimalRoutes() {
        ...
        if (clientPoints.isEmpty() || fleet.isEmpty()) {
            log.warn(
                    "Optimization aborted: No customer points or active vehicles available.");
            return;
        }
        ...
        List<DeliveryPoint> allLocations = prepareLocationsWithDepot(
                this.defaultDepot, clientPoints);
        ...
        log.info(
                "Starting VRP Optimization. Locations: {} (inc. Depot), Active Fleet: {}",
                numLocations, numVehicles);

        // [EN] Delegate matrix building to the distance service
        // [UA] Делегуємо побудову матриці нашому картографічному сервісу
        long[][] distanceMatrix = distanceProvider.createDistanceMatrix(
                allLocations);
        ...
        RoutingIndexManager manager = new RoutingIndexManager(numLocations,
                numVehicles, depotIndex);
        ...
        long[] vehicleWeightCapacities = fleet.stream()
                                              .mapToLong(
                                                      v -> (long) v.getVehicleWeightCapacity())
                                              .toArray();
        ...
        // ===================================================================
        final int palletCallbackIndex = routing.registerUnaryTransitCallback(
                (long fromIndex) -> {
                    int fromNode = manager.indexToNode(fromIndex);
                    return (long) allLocations.get(fromNode)
                                              .getMainWarehousePlaces();
                });

        long[] vehiclePalletCapacities = fleet.stream()
                                              .mapToLong(
                                                      Vehicle::getVehiclePalletCapacity)
                                              .toArray();
        ...
        if (solution != null) {
            ...
            log.error(
                    "Solver failed to find an optimal routing configuration. Check weight/pallet capacities.");
        }
    }

    /**
     * Converts a Depot entity into a graph node and prepends it to the customer list. Перетворює об'єкт Депо на вузол
     * графа та ставить його першим у список клієнтів.
     */
    private List<DeliveryPoint> prepareLocationsWithDepot(Depot depot,
            List<DeliveryPoint> clientPoints) {
    ...
    /**
     * Formats and logs the optimal routing layout found by the mathematical solver. Форматовано виводить у лог
     * результати оптимального планування маршрутів.
     */
    private void printSolution(RoutingModel routing,
        ...
        for (int i = 0; i < fleet.size(); ++i) {
            ...
            routeStr.append(String.format(
                    "Route for Vehicle #%d (%s, Max: %.0fkg/%dpl): Depot",
                    vehicle.getId(), vehicle.getVehicleModel(),
                    vehicle.getVehicleWeightCapacity(),
                    vehicle.getVehiclePalletCapacity()));
            ...
            while (!routing.isEnd(index)) {
                ...
                routeDistance += routing.getArcCostForVehicle(previousIndex,
                        index, i);
                ...
                if (nodeIndex != 0) {
                    ...
                    routeStr.append(" -> ")
                            .append(point.getCustomerName())
                            .append(String.format("(w:%.0f, p:%d)",
                                    point.getWeightKg(),
                                    point.getMainWarehousePlaces()));
                }
            }
            ...
            if (routeDistance > 0) {
                ...
                log.info("Vehicle #{} ({}) - Unused (Stay at Depot)",
                        vehicle.getId(), vehicle.getVehicleModel());
            }
        }
        log.info("=========================================");
