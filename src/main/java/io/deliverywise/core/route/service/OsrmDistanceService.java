...
/**
 * <p>
 * Implementation of the {@link DistanceProvider} that utilizes OSRM (Open Source Routing Machine) API.
 * </p>
 * <p>
 * Реалізація інтерфейсу {@link DistanceProvider}, яка використовує OSRM (Open Source Routing Machine) API.
 * </p>
 *
 * <p>
 * Computes real-world road network distances and travel durations based on OpenStreetMap data.
 * </p>
 * <p>
 * Обчислює реальні відстані дорожньої мережі та тривалість поїздок на основі даних OpenStreetMap.
 * </p>
 *
...
public class OsrmDistanceService implements DistanceProvider {
    ...
    private static final Logger log = LoggerFactory.getLogger(
            OsrmDistanceService.class);

    /**
     * Calculates the distance matrix by executing an HTTP request to the OSRM table service. Розраховує матрицю
     * відстаней шляхом виконання HTTP-запиту до табличного сервісу OSRM.
     *
    ...
    public long[][] createDistanceMatrix(List<DeliveryPoint> points) {
        ...
        // a logistics-manager-defined lockdown area, artificially inflate the distance/time matrix weight to force
        // Google
        // OR-Tools detour.
        ...
        // (наприклад, профілі трафіку для п'ятниці вечора та неділі ранку, зафіксовані на основі історичних шаблонів
        // швидкостей
        // OSM).
        // 2. Віртуальні стіни: Інтегрувати перевірку полігонів із таблиці 'blocked_zones' у PostGIS. Якщо вектор
        // маршруту
        // перетинає заблоковану логістом зону, штучно збільшувати вагу (час/відстань) у матриці, щоб змусити Google
        // OR-Tools
        // шукати об'їзд.
        // Примітка: Оперативні пробки ігноруються навмисно, оскільки водії використовують Waze/Google Maps під час
        // рейсу;
        ...
        for (int i = 0; i < size; i++) {
            ...
            log.info("Point {}: {} | Lat: {}, Lon: {}", i, p.getCustomerName(),
